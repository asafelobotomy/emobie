...
pub(super) const KEYBOARD_READ_RULES: &[u8] =
    include_bytes!("../../../../packaging/udev/98-emobie-keyboard-read.rules");
...
pub(super) const KEYBOARD_READ_RULES_NAME: &str = "98-emobie-keyboard-read.rules";

/// (bytes, install mode, path relative to the staging directory).
pub(super) const STAGED_FILES: [(&[u8], &str, &str); 5] = [
    (SETUP_SCRIPT, "755", "setup-input-access.sh"),
    (UDEV_RULES, "644", UDEV_RULES_NAME),
    (KEYBOARD_READ_RULES, "644", KEYBOARD_READ_RULES_NAME),
